...
class Client {
    ...
    async request(method, url, query, data, options = {}) {
        ...
        this.logger.info(`Request received. Path: ${url}, Endpoint: ${this.endpoint}. Method: ${method}, Headers: ${JSON.stringify(authHeaders)}, Data: ${data}}`);
        ...
        async function retry() {
            ...
            try {
                ...
                if (tryTimes <= maxRetry) {
                    self._logRetryInfo(error.name, tryTimes, { url, headers, data });
                    return await retry();
                }
                self._logRequestError(`Request Timeout Error`, { url, headers, data, response: "" });
            ...
            if (statusCode === 200) { // 请求成功
                try {
                    ...
                    // JSON解析失败，重试
                    if (tryTimes <= maxRetry) {
                        self._logRetryInfo("Response JSON Format Error", tryTimes, { url, headers, data });
                        return await retry();
                    }
                    // 重试结束，抛出异常
                    self._logRequestError("✘ Response JSON Format Error", { url, headers, data, response: response.body });
                ...
                self._logRequestError(`✘ Client Error(${statusCode}) ${statusMsg}`, { url, headers, data, response: response.body });
                throw clientException(statusMsg, `status: ${statusCode}, body: ${response.body}`);

            } else if (statusCode >= 500) { // 服务端错误
                // 服务端错误，重试
                if (tryTimes <= maxRetry) {
                    self._logRetryInfo(`Server Error(${statusCode})`, tryTimes, { url, headers, data });
                    return await retry();
                }
                // 重试结束，抛出异常
                const statusMsg = statusMap[statusCode];
                self._logRequestError(`✘ Server Error(${statusCode}) ${statusMsg}`, { url, headers, data, response: response.body });
                throw serverException(statusMsg, `status: ${statusCode}, body: ${response.body}`);
            } else {
                self._logRequestError(`✘ Unknown Server Error(${statusCode})`, { url, headers, data, response: response.body });
                throw serverException("Unknown Server Error", `Status: ${statusCode}, Url: ${url}, Data: ${data}`);
            }
        }
        return await retry();
    }
    _logRetryInfo(name, tryTimes, json) {
        const headerStr = JSON.stringify(json.headers);
        this.logger.info(`Retry ${name} ${tryTimes} Time => Url: ${json.url}, Header: ${headerStr}, Data: ${json.data}`);
    }
    _logRequestError(name, json) {
        const headerStr = JSON.stringify(json.headers);
        this.logger.error(`${name} => Url: ${json.url}, Header: ${headerStr}, Data: ${json.data}, Response: ${json.response}`);
    }
}
