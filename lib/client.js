...
function formatLog(format, data) {
        ...
        async function retry() {
            ...
            try {
                ...
                if (tryTimes <= maxRetry) {
                    ...
                    self.logger.info(`${logTitle} => ${formatLog(self.logFormat, logData)}`);
                    return await retry();
                }
                self.logger.info(`Request Timeout Error => ${formatLog(self.logFormat, logData)}`);
            ...
            // 网路请求相关日志输出信息
            const logInfo = formatLog(self.logFormat, logData);
            ...
            if (statusCode === 200) { // 请求成功
                try {
                    ...
                    throw serverException(errorMap.SDK_RESPONSE_JSON_FORMAT_ERROR, logInfo);
                }

            } else if (statusCode >= 400 && statusCode < 500) { // 客户端错误
                // 客户端错误，直接抛出异常
                const statusMsg = statusMap[statusCode];
                self.logger.error(`✘ ${statusMsg} => ${logInfo}`);
                throw clientException(statusMsg, logInfo);

            } else if (statusCode >= 500) { // 服务端错误
                const statusMsg = statusMap[statusCode];
                // 服务端错误，重试
                if (tryTimes <= maxRetry) {
                    const logTitle = `Retry <${statusMsg}> ${tryTimes} times`;
                    self.logger.info(`${logTitle} => ${logInfo}`);
                    return await retry();
                }
                // 重试结束，抛出异常
                self.logger.error(`✘ ${statusMsg} => ${logInfo}`);
                throw serverException(statusMsg, logInfo);
            } else {
                self.logger.error(`✘ Unknown Error => ${logInfo}`);
                throw serverException("Unknown Server Error", logInfo);
            }
        }
        return await retry();
    }
