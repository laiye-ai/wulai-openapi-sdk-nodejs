 ...
 */
class Http {
  constructor(config, httpOpts = {}) {
    this.endpoint = config.endpoint;
    this.httpOptions = httpOpts;
  ...
  async request(method, url, query = {}, body = "", headers = {}) {
    ...
    let resBody = "";
    try {
      response = await httpx.request(reqUrl, reqOptions);
      resBody = await httpx.read(response, "utf8");
    } catch (error) {
      throw exception.throwException(error.name, error.message);
    }
    let resJson = null;
    ...
    if (status >= 400 && status < 500) {
      throw exception.clientException(
        `status: ${status}, code: ${errCode}, message: ${errMsg}`,
        {
          status: status,
          result: resJson,
          code: errCode
