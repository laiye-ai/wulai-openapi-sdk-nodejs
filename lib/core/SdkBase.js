...
class Client {
  ...
  constructor(config) {
    ...
    config.apiVersion && (this.apiVersion = config.apiVersion);
    config.options && (this.httpOptions = config.options);
    config.debug !== undefined && (this.debug = config.debug);
    this.httpOptions.maxRetry > 0 && (this.defaultMaxRetry = this.httpOptions.maxRetry)
    this.pubkey = config.pubkey;
    this.secret = config.secret;
    this.logger = createLogger(this.debug);
    this.defaultTimout = this.httpOptions.timeout || 6000;
    this.__http = new Http({ endpoint: this.endpoint });
  }
  ...
  async request(method, url, query, data, options = {}) {
    ...
    // 请求信息打印
    this.debug && this.logger.info(`Request received => ${formatLog(this.logFormat, {
      uri: url,
    ...
    return await this.__autoRetryRequest(
      method,
      url,
      data,
      httpOptions,
  ...
  async __autoRetryRequest(
    ...
    async function retry() {
      ...
      let logData = {
        ...
        uri: url,
        method: method,
        req_header: self.debug && JSON.stringify(headers),
        req_body: data
      };
      try {
        tryTimes++;
